...
TOPIC_KEYS = ["project", "normalized_title"]
DIMENSION_COLUMNS = ["project", "language", "project_family"]
...
def build_top_pages_hourly(page_hourly: DataFrame, top_n: int) -> DataFrame:
    ranking = Window.partitionBy("date", "hour", "project").orderBy(
        F.desc("view_count"),
...
def filter_eligible_topics(
    page_hourly: DataFrame,
    min_topic_views: int,
    min_history_hours: int,
) -> DataFrame:
    """Restrict modeling to topics with enough history and volume for a stable baseline."""
    eligible = (
    ...
    return page_hourly.join(eligible, on=TOPIC_KEYS, how="left_semi")
...
def build_trends_and_anomalies(
    ...
    enriched = (
        ...
        .withColumn(
            ...
            F.percentile_approx("log1p_views", 0.5, percentile_accuracy).over(history),
        )
        .withColumn(
            "log_absolute_deviation",
            F.abs(F.col("log1p_views") - F.col("rolling_baseline_log_median")),
        )
        .withColumn(
            "rolling_baseline_log_mad",
            F.percentile_approx(
                "log_absolute_deviation", 0.5, percentile_accuracy
            ).over(history),
        )
        .withColumn(
            "robust_z_score",
    ...
    trend_rank = Window.partitionBy("date", "hour", "project").orderBy(
        F.desc("trend_score"),
        F.desc("view_count"),
        F.asc("normalized_title"),
    )
    enriched = enriched.withColumn("trend_rank", F.row_number().over(trend_rank))
    enriched = enriched.drop("log_absolute_deviation").persist(StorageLevel.DISK_ONLY)
    anomalies = (
...
def build_forecast_features(
    ...
    return (
        ...
        .withColumn("forecast_history_observed_hours", F.count("view_count").over(history))
        .withColumn(
...
def build_forecast_evaluation(
    ...
    dimensions = ["project", "language", "project_family"]
    methods = [
...
def _write_table(
    ...
    output: Path,
    mode: str,
    partition_columns: tuple[str, ...] = ("date", "project"),
) -> None:
