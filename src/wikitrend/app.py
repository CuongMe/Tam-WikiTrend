...
def load_dashboard_data(database_path: str, validation_report_path: str) -> dict[str, pd.DataFrame]:
    ...
    validation = pd.DataFrame(
        [
            ...
            {
                "metric": "errors",
                "value": len(validation_payload.get("errors", [])),
            },
            {
                "metric": "warnings",
                "value": len(validation_payload.get("warnings", [])),
            },
        ]
    )
...
def filter_frames(
    ...
    return hourly.loc[hourly_mask].copy(), daily.loc[daily_mask].copy(), top_pages.loc[
        top_mask
    ].copy()
...
st.caption("Validated Gold aggregates served from DuckDB views")
...
tabs = st.tabs(["Overview", "Segments", "Top Pages", "Quality"])
...
with tabs[0]:
    ...
    segment_summary["segment"] = (
        segment_summary["project"] + " / " + segment_summary["access_mode"]
    )
...
with tabs[1]:
    ...
    segment_metrics["segment"] = (
        segment_metrics["project"] + " / " + segment_metrics["access_mode"]
    )
...
with tabs[2]:
    top_summary = (
        top_pages.groupby(
            ["project", "access_mode", "normalized_title", "page_title"],
            as_index=False,
        )
        .agg(
    ...
    top_hourly = (
        ...
        .merge(
            hourly[
                ["date", "hour", "project", "access_mode", "total_views"]
            ],
